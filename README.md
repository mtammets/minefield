...
The game includes a production-ready donate flow with:

- dynamic donate availability from `/api/donate/config`
- secure checkout session creation via `/api/donate/checkout-session`
- polished in-game UI entry points (Welcome and Pause screens)
- return-state handling (`?donate=success` / `?donate=cancel`)

### Provider Options

1. Stripe Checkout (recommended)
2. External donate link fallback

### Environment Variables

```bash
# Core toggle (optional: auto-enabled when provider config exists)
DONATE_ENABLED=true

# Stripe provider (recommended)
DONATE_STRIPE_SECRET_KEY=sk_live_or_test_xxx
DONATE_CURRENCY=usd
DONATE_PRESET_AMOUNTS=5,10,25,50
DONATE_MIN_AMOUNT=2
DONATE_MAX_AMOUNT=250
DONATE_AMOUNT_STEP=1
DONATE_SUCCESS_URL=/?donate=success
DONATE_CANCEL_URL=/?donate=cancel
DONATE_ALLOW_PROMO_CODES=false
DONATE_CAMPAIGN_NAME=Support Minefield Drift
DONATE_CAMPAIGN_DESCRIPTION=Help fund new tracks and online updates.
DONATE_PUBLIC_MESSAGE=Support ongoing development with a one-time donation.

# Fallback provider (used when Stripe key is not set)
DONATE_LINK_URL=https://your-site.example/donate
```

Notes:

- `DONATE_*_AMOUNT` values are in major currency units (for example `5` means `$5.00`).
- Stripe mode is selected automatically when `DONATE_STRIPE_SECRET_KEY` is present.
- If Stripe is missing but `DONATE_LINK_URL` exists, the UI opens the external link instead.
- If no provider is configured, donate UI controls stay hidden.
