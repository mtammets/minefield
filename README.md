...
If you want signed-in users to upload profile photos, custom car wraps, garage wrap presets,
editor-managed billboard media, and the showroom intro video through Supabase, configure Supabase
auth plus Storage:
...
# Optional but recommended for automatic schema bootstrap
# (leaderboard, player economy, audio defaults):
SUPABASE_DB_URL=postgresql://...
SUPABASE_DB_POOLER_URL=postgresql://...
# Optional overrides:
SUPABASE_PROFILE_IMAGES_BUCKET=profile-images
SUPABASE_CAR_WRAPS_BUCKET=car-wraps
SUPABASE_GARAGE_WRAP_PRESETS_BUCKET=garage-wrap-presets
SUPABASE_BILLBOARD_MEDIA_BUCKET=billboard-media
SUPABASE_SHOWROOM_INTRO_BUCKET=showroom-intro-media
...
- `supabase/garage-wrap-presets.sql`
...
- garage wrap preset uploads are normalized server-side before publish and stored in `garage-wrap-presets`
- billboard uploads are normalized server-side before publish and stored in `billboard-media`
- showroom intro uploads are normalized server-side and stored in `showroom-intro-media`
...
- The showroom intro video follows the same server-side normalization flow and can be served from
...
- The game now includes a modular Web Audio mixer (`public/js/audio-system.js`) with:
    ...
    - deploy defaults sourced from a Supabase-backed server config row
- When the game is opened through the local dev server (loopback/admin access), changing mixer
  slider values also updates the Supabase audio-defaults record, so the next deploy uses those
  values as live defaults for users without custom local audio prefs.
